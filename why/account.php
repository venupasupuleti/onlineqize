...
{
$eid=@$_GET['eid'];
// Fetch the history of the quiz attempt
...
echo '<div><a href="account.php?q=resultreport&eid='.$eid.'" class="btn btn-info">View Report</a></div>';

}
// View Report Section
if(@$_GET['q']== 'resultreport' && @$_GET['eid']) 
{
$eid = @$_GET['eid'];
// Fetch all questions for the quiz
$q = mysqli_query($con, "SELECT * FROM questions WHERE eid='$eid'") or die('Error157');
echo '<div class="panel"><center><h1 class="title" style="color:#660033">View Report</h1></center><br /><table class="table table-striped title1" style="font-size:20px;font-weight:1000;">';
...
while ($row = mysqli_fetch_array($q)) {
    ...
    while ($row2 = mysqli_fetch_array($q2)) {
        ...
        $q3 = mysqli_query($con, "SELECT * FROM answer WHERE qid='$qid'") or die('Error');
        $row3 = mysqli_fetch_array($q3);
        $correct_option_id = $row3['ansid'];

        if ($optionid == $correct_option_id) {
            echo '<tr style="color:green"><td>'.$option.'</td></tr>';
        } else {
            // Check if the user selected this option and it was wrong
            $q4 = mysqli_query($con, "SELECT * FROM user_answer WHERE qid='$qid' AND eid='$eid' AND email='$email' AND ans='$optionid'") or die('Error');
            if (mysqli_num_rows($q4) > 0) {
                echo '<tr style="color:red"><td>'.$option.'</td></tr>';
            } else {
